                    ...
                    <tr>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div>
                            <img src="{{asset('public/images/'.$student->profile_image)}}" class="avatar avatar-sm me-3 border-radius-lg image-zoom" tabindex="0" alt="user1">
                          </div>
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm text-zoom">{{$student->name}}</h6>
                            <p class="text-xs text-secondary mb-0">#{{$student->id}}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td class="align-middle text-center text-sm">
                          <span class="badge badge-sm {{ $student->status == 'Active' ? 'bg-gradient-success' : 'bg-gradient-danger' }}">
                              {{ $student->status }}
                          </span>
                      </td>
                      <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold">{{$student->start_date}}</span>
                      </td>
                      <td class="align-middle">
                        <div class="col-lg-3 col-md-6 special-grid bulbs brand-id" style="padding-left: 50px;">
                          <div class="products-single fix">
                              <div class="mt-2">
                                  <!-- Edit button -->
                                  <a style="width:110px" href="{{route('edit_student', $student->id)}}" class="btn bg-gradient-warning btn-sm btn1-zoom">
                                 ...
                                 <a style="width:110px" href="{{ route('show_progress', $student->id) }}" class="btn bg-gradient-success btn-sm btn1-zoom">
                                  ...
                                  <form action="{{route('delete_student', $student->id)}}" method="POST" style="display:inline-block;">
                                      @csrf
                                      @method('DELETE')
                                      <button style="width:110px" type="submit" class="btn btn-danger btn-sm btn2-zoom" onclick="return confirm('Are you sure you want to delete this student?');">
